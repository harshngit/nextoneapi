...
-- ─── Add per_day_salary column (migration patch) ─────────────────────────────
-- Run this if table already exists from previous migration
ALTER TABLE employee_salaries
  ADD COLUMN IF NOT EXISTS per_day_salary NUMERIC(12, 2) CHECK (per_day_salary >= 0);

COMMENT ON COLUMN employee_salaries.per_day_salary IS 'Per day salary — either set directly or auto-calculated from monthly_salary / working_days_in_month';
